...
def args_parser():
    ...
    parser.add_argument('--checkpointsmodelfile', type=str, default='./checkpoints/lite_hcnnet/PaviaU/model_96.12.pth')
...
def main():
    ...
    if args.PCA is None:
        model = Lite_HCNet(args.hsi_bands, args.num_class, args.patch_size).to(device)
    else:    
        model = Lite_HCNet(args.PCA, args.num_class, args.patch_size).to(device)
