...
def args_parser():
    ...
    parser.add_argument('--modelfile', type=str, default='./checkpoints/hybirdsn/Houston/model_98.43.pth')
...
def test(model, device, test_loader, args):
    ...
    with torch.no_grad():
        for inputs_1, labels in test_loader:
            ...
            # 添加通道维度，将形状从[batch, channels, height, width]变为[batch, 1, channels, height, width]
...
def main():
    ...
    _, test_loader = build_data_loader_f3d(args)
    ...
    test_time = start_time.elapsed_time(end_time) / 1000  # 转换为秒
    print('\n测试总时间: %.2f 秒' % test_time)
    with open(args.log_file, 'a') as appender:
        appender.write('\n测试总时间: %.2f 秒\n' % test_time)
