...
def args_parser():
    ...
    parser.add_argument('--patch_size', type=int, default=9, help='patch size, need to be divisible by mpatch_size')
...
def train(model, device, train_loader, optimizer, epoch, args):
    ...
    print(' [epoch time: %.2f seconds]' % (epoch_time))  # 打印每轮训练时间
    
    content = ' epoch %d' % (epoch) + ' [loss avg: %.4f]' %( total_loss/(len(train_loader))) + \
              ' [current loss: %.4f]' %(loss.item()) + ' [epoch time: %.2f seconds]' % (epoch_time)
...
def main():
    ...
    train_loader, val_loader = build_data_sim_loader(args)
    ...
    print('Total training time: %.2f seconds (%.2f minutes)' % (total_training_time, total_training_time/60))
    with open(args.log_file, 'a') as appender:
        appender.write('\n')
        appender.write('Total training time: %.2f seconds (%.2f minutes)\n' % (total_training_time, total_training_time/60))
