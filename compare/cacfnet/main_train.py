...
def train(model, device, train_loader, optimizer, epoch, args):
    ...
    print(' [epoch time: %.2f seconds]' %(epoch_time))  # 打印训练时间
    
    content = ' epoch %d' % (epoch) + ' [loss avg: %.4f]' %( total_loss/(len(train_loader))) + ' [current loss: %.4f]' %(loss.item()) + ' [epoch time: %.2f seconds]' %(epoch_time)
