...
import sys
sys.path.append('.')
from compare.msdan.Msdan import msdan
...
def args_parser():
    ...
    parser.add_argument('-dataset', type=str, default='Houston',
                        choices=['PaviaU', 'Houston','Salinas','WHU_Hi_LongKou'])
    ...
    parser.add_argument('--modelfile', type=str, default='./checkpoints/msdan/Houston/model_98.48.pth')
...
def main():
    ...
    if args.PCA is None:
        model = msdan(args.hsi_bands, args.num_class).to(device)
    else:    
        model = msdan(args.PCA, args.num_class).to(device)
    model.load_state_dict(torch.load(args.modelfile))
    test(model, device, test_loader, args)
    end_time = time.time()  # 新增
    print(f"测试用时: {end_time - start_time:.2f} 秒")  # 新增
