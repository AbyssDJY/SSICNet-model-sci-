...
import argparse
from compare.f3d.F3Dnet import F3D
...
def args_parser():
    ...
    parser.add_argument('--num_class', type=int, default=16, help='类别数')
...
def train(model, device, train_loader, optimizer, epoch, args):
    ...
    for i, (inputs_1, labels) in enumerate(train_loader):
        ...
        if args.PCA is not None:
            # 将输入重塑为[batch_size, 1, channels, height, width]格式
        ...
        # 打印输入形状，用于调试
        if i == 0 and epoch == 0:
            print(f"Input shape: {inputs_1.shape}")
            
        optimizer.zero_grad()
        
        # 使用try-except捕获形状错误并提供更详细的信息
        try:
            outputs = model(inputs_1)
        except RuntimeError as e:
            if "Kernel size can't be greater than actual input size" in str(e):
                print(f"Error: Input tensor is too small for convolution operations.")
                print(f"Input shape: {inputs_1.shape}")
                print(f"Consider using a larger patch size or reducing the kernel sizes in the model.")
            raise
            
        # 检查标签范围
        assert labels.min() >= 0 and labels.max() < args.num_class, f"标签超出范围: min={labels.min()}, max={labels.max()}, num_class={args.num_class}"
...
def main():
    ...
    train_loader, val_loader = build_data_loader_f3d(args)

    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    model = F3D(args.num_class).to(device)
    # 输出模型参数量
    model.print_model_params()
    
    # 打印模型结构
    print(f"F3D模型结构已创建，类别数: {args.num_class}")
